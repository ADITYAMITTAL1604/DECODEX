...
export default function ConsentConfirm() {
  ...
  if (loading) return <ConsentShell><p className="font-body text-on-surface-variant">Checking your consent link…</p></ConsentShell>;
  if (error && !tokenData) return <ConsentShell><ErrorState message={error} /></ConsentShell>;
  if (!tokenData) return null;
  if (confirmed) return <ConsentShell><div className="rounded-2xl bg-primary-fixed p-7 text-on-primary-fixed"><span className="material-symbols-outlined text-4xl" aria-hidden="true">check_circle</span><h1 className="mt-3 font-display text-3xl font-bold">Consent confirmed</h1><p className="mt-2 font-body text-lg">{tokenData.student.display_name} can now use Decodex recording features. You may close this page.</p></div></ConsentShell>;

  return (
    <ConsentShell>
      <p className="font-display text-sm font-bold uppercase tracking-[0.12em] text-secondary">Parent consent</p>
      <h1 className="mt-2 font-display text-3xl font-bold text-primary">Confirm consent for {tokenData.student.display_name}</h1>
      <p className="mt-2 font-body text-lg text-on-surface-variant">{tokenData.student.grade_level ? `Grade ${tokenData.student.grade_level}` : 'Student account'}</p>

      <form onSubmit={submitConsent} className="mt-8">
        ...
        <div className="mt-7 flex flex-col gap-2">
          ...
          <input id="child-dob" type="date" value={dateOfBirth} onChange={(event) => setDateOfBirth(event.target.value)} className="h-14 rounded-xl border-2 border-surface-variant bg-surface px-4 font-body text-lg text-on-surface outline-none focus:border-primary" required />
          {attemptsRemaining !== null ? <p className="font-body text-sm text-on-surface-variant">{attemptsRemaining} verification attempt{attemptsRemaining === 1 ? '' : 's'} remaining.</p> : null}
        </div>
...
function ConsentShell({ children }: { children: ReactNode }) {
  return <main className="min-h-screen bg-background px-container-padding py-12 text-on-surface"><div className="mx-auto w-full max-w-[680px] rounded-2xl bg-surface-container-lowest p-7 shadow-[0_16px_32px_rgba(45,41,38,0.05)] sm:p-10">{children}</div></main>;
}

function ErrorState({ message }: { message: string }) {
  return <div className="rounded-2xl bg-error-container p-7 text-on-error-container"><span className="material-symbols-outlined text-4xl" aria-hidden="true">error</span><h1 className="mt-3 font-display text-2xl font-bold">Consent link unavailable</h1><p className="mt-2 font-body">{message}</p></div>;
}
